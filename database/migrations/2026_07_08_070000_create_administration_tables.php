...
return new class extends Migration
{
    ...
    {
        Schema::create('business_profiles', function (Blueprint $t) {
            $t->id();
            $t->foreignId('branch_id')->unique()->constrained()->cascadeOnDelete();
            $t->string('name');
            $t->string('phone', 30)->nullable();
            $t->text('address')->nullable();
            $t->string('logo_path')->nullable();
            $t->string('primary_color', 20)->default('#cf4b32');
        ...
        Schema::create('settings', function (Blueprint $t) {
            ...
            $t->string('type')->default('json');
            $t->timestamps();
            $t->unique(['branch_id', 'key']);
        });
        Schema::create('order_documents', function (Blueprint $t) {
            $t->id();
            $t->foreignId('order_id')->constrained()->cascadeOnDelete();
            $t->foreignId('user_id')->constrained();
            $t->enum('type', ['customer_html', 'customer_pdf', 'customer_image', 'kitchen', 'delivery']);
            $t->string('path')->nullable();
            $t->longText('content')->nullable();
            $t->timestamps();
        });
        Schema::create('cash_days', function (Blueprint $t) {
            $t->id();
            ...
            $t->decimal('expected_amount', 12, 2)->default(0);
        ...
        Schema::create('cash_movements', function (Blueprint $t) {
            $t->id();
            $t->foreignId('cash_day_id')->constrained()->cascadeOnDelete();
            $t->foreignId('user_id')->constrained();
    ...
    {
        foreach (['audit_logs', 'report_snapshots', 'daily_reports', 'cash_movements', 'cash_days', 'order_documents', 'settings', 'business_profiles'] as $x) {
